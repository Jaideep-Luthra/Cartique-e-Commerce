...
export default function Products() {
  ...
  function fetchProduct() {
    ApiServices.customerViewAllProduct({ status: true })
      .then((res) => {
        if (res.data.success) {
  ...
  const [foundProduct, setFoundProduct]=useState("1")
  useEffect(() => {
  ...
  const processVoiceCommand = (speechText) => {
    const lowerCaseText = speechText.toLowerCase();

    if (lowerCaseText.startsWith("search")) {
      const query = lowerCaseText.replace("search", "").trim();
      fetchResults(query);
    } else if (lowerCaseText.startsWith("add")) {
      const query = lowerCaseText.replace("add", "").replace("to cart", "").trim();
      addProductToCartByName(query);
    } else {
      toast.error("Unknown command! Try 'Search [product name]' or 'Add [product name] to cart'");
    }
  };

  const addProductToCartByName = (productName) => {
    setFoundProduct(productData.find((product) => 
            ...
            <div className="row">
              ...
              <div className="col-md-4">
                <div style={{ padding: "2rem" }}>
                  <button onClick={listening ? stopListening : startListening} className={`btn ${listening ? "btn-danger" : "btn-primary"} float-end`}>
                    {listening ? <i className="fa fa-circle"></i> : <i className="fa fa-microphone"></i>}
                  </button>
